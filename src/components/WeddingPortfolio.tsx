...
import { CalendarDays, ChevronRight, HeartHandshake, Plus, Settings, Users } from 'lucide-react';
...
function ProfileSheet({ onClose }: { onClose: () => void }) {
  ...
  return <div className="sheet-backdrop" onClick={onClose}><section className="sheet profile-sheet" onClick={event => event.stopPropagation()}><div className="sheet-heading"><div><p className="eyebrow">Your profile</p><h2>Make each wedding work for you</h2></div><button className="close-button" onClick={onClose}>×</button></div><label>Your name<input value={name} onChange={event => setName(event.target.value)} /></label><label>Date of birth <span>Optional</span><input type="date" value={dateOfBirth} onChange={event => setDateOfBirth(event.target.value)} /></label><label>Meal preference<select value={mealPreference} onChange={event => setMealPreference(event.target.value)}><option value="vegetarian">Vegetarian</option><option value="non_vegetarian">Non-vegetarian</option><option value="vegan">Vegan</option><option value="jain">Jain</option><option value="no_preference">No preference</option></select></label><button className="primary-button" onClick={() => { updateMyProfile({ name: name.trim() || 'Member', dateOfBirth: dateOfBirth || undefined, mealPreference: mealPreference || undefined }); onClose(); }}>Save profile</button></section></div>;
}
...
export default function WeddingPortfolio({ onOpenWedding }: Props) {
  ...
  useEffect(() => {
    ...
    const planIsLive = mine.some(({ wedding }) =>
      wedding.primaryName === newPlan.primaryName &&
      wedding.partnerName === newPlan.partnerName &&
  ...
  if (mine.length === 0 && !newPlan && !inviteCode) return <><NewWeddingHome name={me?.name} onCreate={() => setCreating(true)} onProfile={() => setProfileOpen(true)} onInvite={() => { const code = window.prompt('Enter your wedding invite code'); if (code?.trim()) setInviteCode(code.trim()); }} />{profileOpen && <ProfileSheet onClose={() => setProfileOpen(false)} />}</>;
  return <main className="portfolio-page"><header className="portfolio-header"><div><p className="eyebrow">Welcome{me?.name ? `, ${me.name}` : ''}</p><h1>Your weddings</h1></div><button className="avatar-button" aria-label="Open your profile" onClick={() => setProfileOpen(true)}>{me?.name?.slice(0, 1).toUpperCase() || <Settings size={18}/>}</button></header><p className="portfolio-intro">Choose a wedding to see what needs you, or start a new plan.</p>{inviteCode && <section className="join-invite"><b>You’ve been invited to a wedding</b><p>Join with the role chosen for you by the wedding owner.</p><button className="primary-button" onClick={acceptInvite}>Join this wedding</button>{inviteError && <span>{inviteError}</span>}</section>}<section className="wedding-list" aria-live="polite">{newPlan && <div className="wedding-card" aria-label={`${newPlan.primaryName} and ${newPlan.partnerName}'s wedding plan is being set up`}><span className="wedding-card-mark"><CalendarDays size={22}/></span><span className="wedding-card-copy"><b>{newPlan.primaryName} & {newPlan.partnerName}</b><span>{newPlan.city} · {newPlan.dateLabel}</span><small>Setting up your plan…</small></span></div>}{mine.map(({ member, wedding }) => <button className="wedding-card" key={String(wedding.id)} onClick={() => onOpenWedding(wedding.id)}><span className="wedding-card-mark"><CalendarDays size={22}/></span><span className="wedding-card-copy"><b>{wedding.primaryName} & {wedding.partnerName}</b><span>{wedding.city} · {wedding.dateLabel}</span><small><Users size={13}/>{member.role === 'family' ? `${member.side === 'bride' ? 'Bride' : member.side === 'groom' ? 'Groom' : 'Family'} side` : member.role}</small></span><ChevronRight size={20}/></button>)}</section>{mine.length === 0 && !newPlan && <section className="portfolio-empty"><CalendarDays size={28}/><h2>No wedding plans yet</h2><p>When someone adds you to a wedding, it will appear here. You can also create one for your family.</p></section>}<button className="create-wedding-button" onClick={() => setCreating(true)}><Plus size={20}/> Create a wedding plan</button>{profileOpen && <ProfileSheet onClose={() => setProfileOpen(false)} />}</main>;
}
