...
function PortfolioLineArt() {
  return <div className="portfolio-line-art" aria-hidden="true">
    <svg viewBox="0 0 520 260" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path className="portfolio-line-art__arch" d="M75 238V136C75 62 134 24 214 24C294 24 354 62 354 136V238" />
      <path className="portfolio-line-art__fine" d="M98 238V140C98 78 147 47 214 47C281 47 331 78 331 140V238" />
      <path className="portfolio-line-art__stem" d="M398 240C397 180 415 121 467 70" />
      <path className="portfolio-line-art__stem" d="M420 183C448 185 472 170 491 145M410 151C388 145 373 131 362 110M444 110C461 109 475 101 487 88" />
      <path className="portfolio-line-art__leaf" d="M420 183C434 158 462 160 491 145C473 175 446 187 420 183ZM410 151C384 148 371 130 362 110C386 114 404 128 410 151ZM444 110C454 92 474 91 487 88C478 106 462 113 444 110Z" />
      <g className="portfolio-line-art__flower">
        <circle cx="398" cy="151" r="7" />
        <path d="M398 140C384 123 390 108 398 104C406 108 412 123 398 140ZM409 151C426 137 440 143 444 151C440 159 425 165 409 151ZM398 162C384 179 369 173 365 165C369 157 384 151 398 162ZM387 151C370 137 376 122 384 118C392 122 398 137 387 151Z" />
      </g>
      <g className="portfolio-line-art__spark">
        <path d="M194 98V117M184.5 107.5H203.5M438 48V61M431.5 54.5H444.5M466 207V218M460.5 212.5H471.5" />
      </g>
      <path className="portfolio-line-art__ground" d="M44 238H486" />
    </svg>
    <span>Made for the whole family</span>
  </div>;
}
...
function InvitePeople({ weddingId }: { weddingId: bigint }) {
  ...
  return <section className="invite-people"><div className="invite-people-intro"><p className="eyebrow">Invite family</p><h2>Share the plan</h2><p>Choose a role first. They will join with that role and can’t change it themselves.</p></div>{link ? <div className="invite-link"><code>{link}</code><button type="button" onClick={() => navigator.clipboard?.writeText(link)}><Copy size={16} /> Copy link</button></div> : <><div className="invite-role-fields"><label>Role<select value={role} onChange={event => setRole(event.target.value)}><option value="" disabled>Choose a role</option><option value="couple">Couple</option><option value="planner">Event creator</option><option value="family">Family</option><option value="guest">Guest</option></select></label><label>Side <span>Optional</span><select value={side} onChange={event => setSide(event.target.value)}><option value="">No side yet</option><option value="bride">Bride’s side</option><option value="groom">Groom’s side</option></select></label></div><button className="primary-button" type="button" disabled={!role} onClick={makeInvite}>Create invite link</button></>}</section>;
}
...
export default function WeddingPortfolio({ onOpenWedding }: Props) {
  ...
  return <main className="portfolio-page portfolio-page--active"><header className="portfolio-header"><div><p className="eyebrow">Welcome{me?.name ? `, ${me.name}` : ''}</p><h1>Your weddings</h1></div><button className="avatar-button" aria-label="Open your profile" onClick={() => setProfileOpen(true)}>{me?.name?.slice(0, 1).toUpperCase() || <Settings size={18}/>}</button></header><p className="portfolio-intro">Open a shared plan, or create a new one when you are ready.</p>{inviteCode && <section className="join-invite"><b>{invitePreview ? `${invitePreview.brideName} & ${invitePreview.groomName}` : 'You’ve been invited to a wedding'}</b>{invitePreview ? <p>{invitePreview.city} · {invitePreview.dateLabel}<br />Join as {invitePreview.role === 'planner' ? 'event creator' : invitePreview.role}{invitePreview.side ? ` · ${invitePreview.side === 'bride' ? 'Bride’s side' : 'Groom’s side'}` : ''}.</p> : <p>Checking the wedding details…</p>}<button className="primary-button" onClick={acceptInvite}>Join this wedding</button>{inviteError && <span>{inviteError}</span>}</section>}<div className="portfolio-body"><section className="portfolio-weddings" aria-labelledby="your-plans-title"><div className="portfolio-section-heading"><p className="eyebrow">Your plans</p><h2 id="your-plans-title">Choose a wedding to continue</h2></div><div className="wedding-list" aria-live="polite">{newPlan && <div className="wedding-card" aria-label={`${newPlan.brideName} and ${newPlan.groomName}'s wedding plan is being set up`}><span className="wedding-card-mark"><CalendarDays size={22}/></span><span className="wedding-card-copy"><b>{newPlan.brideName} & {newPlan.groomName}</b><span>{newPlan.city} · {newPlan.dateLabel}</span><small>Setting up your plan…</small></span></div>}{mine.map(({ member, wedding }) => <button className="wedding-card" key={String(wedding.id)} onClick={() => onOpenWedding(wedding.id)}><span className="wedding-card-mark"><CalendarDays size={22}/></span><span className="wedding-card-copy"><b>{wedding.brideName} & {wedding.groomName}</b><span>{wedding.city} · {wedding.dateLabel}</span><small><Users size={13}/>{member.role === 'family' ? `${member.side === 'bride' ? 'Bride' : member.side === 'groom' ? 'Groom' : 'Family'} side` : member.role === 'planner' ? 'Event creator' : member.role}</small></span><ChevronRight size={20}/></button>)}</div></section><aside className="portfolio-actions" aria-label="Wedding actions">{mine.filter(({ member }) => member.role === 'couple' || member.role === 'planner').map(({ wedding }) => <InvitePeople key={`invite-${wedding.id}`} weddingId={wedding.id} />)}<div className="portfolio-create"><div><b>Starting another plan?</b><span>Create a separate wedding space when you need one.</span></div><button className="create-wedding-button" onClick={() => setCreating(true)}><Plus size={18}/> Create a wedding plan</button></div><PortfolioLineArt /></aside></div>{profileOpen && <ProfileSheet initialName={googleName} onClose={() => setProfileOpen(false)} />}</main>;
}
