...
function ProfileSheet({ onClose, initialName, required }: { onClose: () => void; initialName: string; required?: boolean }) {
  ...
  return <div className="sheet-backdrop" onClick={required ? undefined : onClose}><section className="sheet profile-sheet" onClick={event => event.stopPropagation()}><div className="sheet-heading"><div><p className="eyebrow">Your profile</p><h2>Before we start, tell us a little about you</h2></div>{!required && <button className="close-button" onClick={onClose}>×</button>}</div><p className="profile-help">Your Google name is filled in below. Confirm or edit it, then add the details that help us plan for you.</p><label>Your name<input value={name} onChange={event => setName(event.target.value)} /></label><label>Date of birth <span>Optional</span><input type="date" value={dateOfBirth} onChange={event => setDateOfBirth(event.target.value)} /></label><label>Gender <span>Optional</span><select value={gender} onChange={event => setGender(event.target.value)}><option value="">Prefer not to say</option><option value="woman">Woman</option><option value="man">Man</option><option value="non_binary">Non-binary</option><option value="prefer_not_to_say">Prefer not to say</option></select></label><label>Meal preference<select value={mealPreference} onChange={event => setMealPreference(event.target.value)}><option value="vegetarian">Vegetarian</option><option value="non_vegetarian">Non-vegetarian</option><option value="vegan">Vegan</option><option value="jain">Jain</option><option value="no_preference">No preference</option></select></label><button className="primary-button" disabled={!name.trim()} onClick={() => { updateMyProfile({ name: name.trim(), dateOfBirth: dateOfBirth || undefined, gender: gender || undefined, mealPreference: mealPreference || undefined }); onClose(); }}>Continue</button></section></div>;
}
...
function InvitePeople({ weddingId }: { weddingId: bigint }) {
  ...
  const makeInvite = () => {
    ...
    createInvitation({ weddingId, code, role: 'family', side: undefined });
    setLink(`${window.location.origin}${window.location.pathname}?invite=${encodeURIComponent(code)}`);
  };
  return <section className="invite-people"><div><p className="eyebrow">Next step</p><h2>Invite your family</h2><p>Send a private link. Each person joins with a role, and can see only the plan they are part of.</p></div>{link ? <div className="invite-link"><code>{link}</code><button type="button" onClick={() => navigator.clipboard?.writeText(link)}><Copy size={16} /> Copy link</button></div> : <button className="primary-button" type="button" onClick={makeInvite}>Create an invite link</button>}</section>;
}

function InviteCodeSheet({ onClose, onContinue }: { onClose: () => void; onContinue: (code: string) => void }) {
  const [code, setCode] = useState('');
  return <div className="sheet-backdrop" onClick={onClose}><section className="sheet invite-code-sheet" onClick={event => event.stopPropagation()}><div className="sheet-heading"><div><p className="eyebrow">Join a wedding</p><h2>Enter your invite code</h2></div><button className="close-button" onClick={onClose}>×</button></div><p>Paste the private code shared by the couple or planner.</p><label>Invite code<input autoFocus value={code} onChange={event => setCode(event.target.value)} placeholder="Paste your code" /></label><button className="primary-button" disabled={!code.trim()} onClick={() => onContinue(code.trim())}>Continue</button></section></div>;
}
...
export default function WeddingPortfolio({ onOpenWedding }: Props) {
  ...
  const profileRequired = !me || me.profileState !== 'confirmed';
  useEffect(() => {
    if (!newPlan) return;
  ...
  if (profileRequired) return <ProfileSheet required initialName={googleName} onClose={() => setProfileOpen(false)} />;
  if (creating) return <Onboarding profileName={me.name.startsWith('Guest ') ? googleName : me.name} onComplete={plan => { setNewPlan(plan); setCreating(false); }} />;
  const acceptInvite = () => {
